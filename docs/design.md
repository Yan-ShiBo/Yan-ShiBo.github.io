...
- 项目页的三个 `.project-case` 是代表案例，不是普通目录卡片。桌面每项使用约 `55% / 45%` 的正文—系统路径图双栏，统一保持正文在左、路径图在右；路径图用四个编号节点说明系统输入、处理与公开输出，不伪装为截图或运行结果。`<= 833px` 时案例回落为单列，正文先于路径图，长英文、按钮和标签均不得撑宽页面。
...
- `.resume-layout` 在宽屏继续使用 `minmax(280px, 320px)` 侧栏与 `minmax(0, 1fr)` 可收缩主体；Hero、锚点和材料区的收紧不得借机扩大侧栏。主体条目以固定日期列和自然内容列构成平面履历，而不是两列普通卡片。简历 Hero 姓名收敛到最大 `68px`，正文通过 `.resume-page` 独立使用 `16px` 基准，不降低全站字号。`<= 1068px` 回落为单列，`<= 640px` 再把日期移到条目正文上方。
...
- 简历正文照片使用 `assets/profile/resume-photo.jpg`，仅服务中英文简历可见内容；全站 `Person.image`、社交图和其他页面头像继续使用 `assets/profile/photo.jpg`，不得因单页改版静默扩大照片替换范围。
...
简历页在 `<= 1068px` 将侧栏与主体改为单列，并在 `641–1068px` 把侧栏内部组织为“照片 + 两列元信息”；`<= 640px` 恢复自然纵向侧栏，同时把正式照片与身份说明组成紧凑横排，把履历日期、条目标题和角色说明改为纵向阅读。六项锚点只在自身容器横向滚动，三项材料链接自然换行或纵向排列；侧栏联系方式的值、关键词、论文标题、组织名称和长小按钮都必须在父容器内换行。既有收缩保护在全部视口生效，不放进条件规则重复定义。
