...
- 首页 Hero 的两栏共用一张连续的基础画布与 `44px` 低密度网格，不为左右栏分别铺设白灰底板；研究标签保持内容宽度。右栏外层使用 `.hero-side-surface` 作为背景语义钩子，布局与背景不得依赖它在 Hero 中的子元素序号。灰色章节从后续概览开始，避免宽屏出现全高竖向色块、单列状态出现与概览粘连的长灰带。
