...
- 首页 Hero 的两栏共用一张连续的基础画布与 `44px` 低密度网格，不为左右栏分别铺设白灰底板；研究标签保持内容宽度。灰色章节从后续概览开始，避免宽屏出现全高竖向色块、单列状态出现与概览粘连的长灰带。
- 内页 Hero 不设置视口比例最小高度，由标题、说明、操作和响应式内边距共同决定自然高度；不得用固定高度、额外高度下限或 `max-height` 人为补空白或裁切长英文和窄屏内容。
- 宽屏可以并列，窄屏按自然阅读顺序变为单列或紧凑网格。
- 首页在 `<= 640px` 把 `.hero-side` 保留为居中吸附的“档案式”横向滑轨，而不是改成纵向堆叠。现有四张直属卡片统一使用 `calc(100% - 56px)` 宽度、`216px` 高度、`28px` 两端留白和 `12px` 间距；首卡居中时稳定露出下一卡 `16px`，卡片之间继续按中心吸附。右侧区块的背景网格放宽到 `48px`，避免窄屏出现过密纹理。
- 移动首页身份卡把地点和邮箱显示为无次级外框的信息行；研究关键词使用一个 2×3 分隔矩阵；站点统计使用一个三栏分段面板。唯一的 `.meta-card--stats` 必须直接包含唯一的 `.stats-grid.hero-stats`，并把横向内边距收紧到 `8px`；每个 `.compact-stat` 使用 `10px 2px` 内边距，为三栏数字保留稳定宽度。中英文标签、关键词和联系方式允许在卡片内换行，统计标签可换行，公开计数使用窄屏自适应的等宽数字，不得撑出分栏或页面。
...
章节底色与卡片材质承担不同层级，而且必须由内容职责显式指定，不得通过 `:nth-of-type` 等 DOM 奇偶规则推导。普通 `.section-block` 使用基础画布；`.section-muted` 只标记用于分组、补充或证明材料的中性章节；`.tile-dark` 保留给极少数真正承担收尾或强调职责的章节，当前正文中仅首页引文使用。普通内页统一从浅灰 `.page-hero` 进入白色主章节，再按内容关系显式穿插浅灰辅助章节；研究页不再在正文中插入深色块。首页单独形成整幅白色 Hero、浅灰概览、深色引文的三段收尾节奏，不在 Hero 内部提前切出灰色半幅。深色主题沿用相同语义，以黑色、弱化深灰和强调深灰区分三层。只有可独立扫描、比较或操作的信息才使用卡片，简单正文不额外套框，也不连续嵌套同材质卡片。
...
- `.project-grid` 使用带最小卡片宽度的 `flex-wrap` 表达可持续增长的项目目录：单项和不满一行的末组自动铺满，多项随可用宽度形成多列；`.project-grid--featured` 提高证明型项目的最小宽度。
...
移动导航的唯一判定谓词是 `(max-width: 833px)`；查询为 false 即进入桌面状态，因此页面缩放产生的 `833px < width < 834px` 小数 CSS 视口也必须清理抽屉。整数边界仍同时检查 833px 和 834px，不能把 834px 写成移动导航状态。该移动谓词内，品牌链接、语言链接、页头和抽屉图标按钮、返回顶部按钮及小按钮的稳定触控目标不得小于 `44px`；响应式调整还应覆盖中英文长文本、键盘焦点和横向滚动。

首页移动 Hero 的卡片规格、内部组合与横向吸附以[首页 Hero 设计合同](#32-hero)为准；`<= 640px` 的通用缩进不得覆盖这套滑轨留白或重新引入不同卡高。
...
研究页的背景、意义和方向卡片使用 `.grid-2`、`.grid-3`、`.grid-4`；项目目录按类别使用 `.project-grid`，条目增加时无需修改固定列数，并在窄屏自然回落为单列。带获奖或证明材料的工程项目使用独立置顶组，避免与无媒体短卡共享同一行；普通项目末行自然填满，卡片内操作入口在同一行底部对齐。项目标题、技术标签和按钮必须允许在窄屏收缩或换行；证明图仍通过现有 Lightbox 查看原图。
...
- 首页研究与工程概览使用唯一的 `h2#home-overview-title.visually-hidden`，由所在 `section` 的 `aria-labelledby` 引用；视觉上不新增标题行，但保留章节语义。
- 视觉字号不能替代语义层级。
