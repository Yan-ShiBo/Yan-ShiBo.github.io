...
- Escape、遮罩和导航点击都能关闭。
- 关闭后焦点回到菜单按钮。
- 设计合同要求从移动断点进入桌面断点时清理遗留状态；当前实现尚未监听这一跨越，见 [M-009](maintenance.md#m-009-移动抽屉缺少跨断点状态清理)。
...
必须同时检查 833px 和 834px，不能把 834px 写成移动导航状态。响应式调整应覆盖中英文长文本、键盘焦点、横向滚动和触屏点击目标。
...
2. 新样式写入 `site.css`；不要新增内联样式。当前个人页与 404 的既有例外不在文档重构中修改。
3. 新交互优先进入 `site.js`；页面内联脚本仅保留已记录例外。
