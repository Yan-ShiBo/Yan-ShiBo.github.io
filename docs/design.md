...
- 首页在 `<= 640px` 把 `.hero-side` 保留为居中吸附的“档案式”横向滑轨，而不是改成纵向堆叠。现有四张直属卡片统一使用 `calc(100vw - 32px)` 宽度、`216px` 高度、`16px` 两端留白和 `12px` 间距，卡片之间按中心吸附；右侧区块的背景网格放宽到 `48px`，避免窄屏出现过密纹理。
- 移动首页身份卡把地点和邮箱显示为无次级外框的信息行；研究关键词使用一个 2×3 分隔矩阵；站点统计使用一个三栏分段面板。中英文标签、关键词和联系方式允许在卡片内换行，统计标签可换行，公开计数使用窄屏自适应的等宽数字，不得撑出分栏或页面。
