...
- 简历 Hero 直接展示姓名、当前身份、研究方向摘要与 PDF/学校邮箱两个主要操作，不再用“简历、成绩单与公开材料”充当抽象页名，也不在 Hero 与侧栏重复放置照片。姓名是该页唯一的高强度展示元素；成绩单继续留在文末公开材料区。
...
档案与简历采用“外层归档容器、内层编辑分隔”的层级：档案概览使用上下发丝线与列间分隔，时间线保留 `.stage-card` 外框；简历保留 `.resume-sidebar` 和 `.resume-main` 外框。简历侧栏放置专用 3:4 正式照片、学校邮箱、主页、GitHub、研究关键词与四项履历速览；主体以五个编号章节和日期列组成“履历证据脊柱”，依次覆盖教育、科研、项目与实习、荣誉、组织与实践。上述容器内部的阶段详情、故事、侧栏元信息及履历条目使用透明底和顶部细分隔，不再重复形成完整卡框；头像、证明材料、按钮、论文状态与标签仍保持各自边界和交互职责。
...
- `.resume-layout` 在宽屏使用 `280–320px` 侧栏与可收缩主体；主体条目以固定日期列和自然内容列构成平面履历，而不是两列普通卡片。`<= 1068px` 回落为单列，`<= 640px` 再把日期移到条目正文上方。
...
简历页在 `<= 1068px` 将侧栏与主体改为单列，并在 `641–1068px` 把侧栏内部组织为“照片 + 两列元信息”；`<= 640px` 恢复自然纵向侧栏，同时把正式照片与身份说明组成紧凑横排，把履历日期、条目标题和角色说明改为纵向阅读。材料网格在 `<= 640px` 使用单列，文档卡片保持 `min-width: 0`，成绩单证明图继续在 `.proof-grid` 内横向滚动；侧栏联系方式的值、关键词、论文标题、组织名称和长小按钮都必须在父容器内换行。既有收缩保护在全部视口生效，不放进条件规则重复定义。
...
- 在线简历以当前获授权 PDF 为内容事实源：学校邮箱、教育、科研论文状态、项目、实习、荣誉和组织经历按 PDF 与已确认补充事实同步；PDF 未列出的电话、微信和旧履历摘要不继续残留。英文页保持相同事实与入口，但按自然学术英语表达。
