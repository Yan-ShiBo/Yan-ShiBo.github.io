...
- 研究页与项目页继续复用全站 `.page-hero`、`.section-block` 和颜色/字体令牌；研究页新增组件以 `.research-page` 限定作用域，不创建独立字体、脚本或脱离双主题的页面级视觉子系统。
- 研究页按“背景—核心问题—研究现状—研究意义—成果”组织。核心问题使用真实有序列表呈现控制器合成、概率保证、可验证性和可扩展性；研究现状使用发丝线分隔的编辑式条目，不把并列研究脉络伪装成流程，也不恢复四张宽泛方向卡片。
- 研究页只把状态空间中的可达—规避任务作为主视觉；背景闭环对象保持低对比度。公式和状态空间图只允许在自身容器内横向滚动，论文与项目行必须允许长英文安全换行，匿名在投稿件只能显示公开主题与投稿类别。项目页继续按稳定类别组织，项目数量与仓库、图片、论文等可选入口不得写死在布局中。
...
- CBF-Rover 必须区分在线控制障碍函数与形式化随机障碍证书：当前可公开事实是以障碍证书为研究对象、以 exact 与 Gazebo 为验证环境的 math-first 工程，以及已经实现并通过经验门禁验证的动态道路在线 CBF 安全过滤；固定几何 q-SBC 仍为 `not_ready`，不能写成“完成障碍证书合成与独立校验”，也不能把 CBF/Gazebo 经验结果扩大为 reach-avoid 概率证明。learned RGB-D 只是默认禁用的经验基线，不作为当前视觉强化学习或硬件部署主线。
