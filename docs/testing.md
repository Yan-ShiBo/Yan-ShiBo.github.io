...
- MicFamily / KTV 技术栈包含 `uni-app`。在线简历采用与原 PDF 对齐的紧凑摘要，显示 `2022.06—2022.08` 实习期、团队开发联调、Spring Boot、MySQL、`uni-app` 与 94 分；`2022.06—2022.07` 项目开发期继续由档案和项目页承担，不强塞回在线简历摘要。
...
- CBF-Rover 的在线 CBF 安全过滤与 fixed-geometry q-SBC 分开陈述：前者可写 exact/Gazebo 中的经验验证，后者仍为 `not_ready`；不得出现“已完成 q-SBC/障碍证书合成与独立校验”或把经验闭环写成形式化概率证明。该边界必须同步到中英文简历、档案项目卡、项目页可见文案、项目 JSON-LD 和验证器事实表，不再公开 Jetson Nano 或嵌入式硬件闭环已经完成的旧表述。
