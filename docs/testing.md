...
简历页还需在中英文 `320px`、`375px`、`640px`、`833px`、`834px`、`1068px`、`1069px` 与 `1440px` 检查：`documentElement.scrollWidth === clientWidth`；`>= 1069px` 为 `280–320px` 侧栏与主体双栏，`<= 1068px` 回落单列且侧栏不保持 sticky；`641–1068px` 的侧栏为正式照片与两列元信息，`<= 640px` 改为紧凑照片/身份横排和自然纵向元信息。主体必须按 01–05 顺序显示教育、科研、项目与实习、荣誉、组织与实践，桌面日期列清晰，`<= 640px` 日期位于正文上方；中英文长论文标题、组织名称、角色说明、学校邮箱、关键词和长按钮均落在父容器内。简历 Hero 与侧栏使用 PDF 中的学校邮箱和已批准入口，不重新出现旧电话或微信；全站页脚与结构化数据继续遵守既有公共身份合同。`.doc-grid` 不产生自身宽度溢出；`.proof-grid` 保留 `overflow-x: auto` 的内部滑轨；3:4 正式照片不变形，竖版 PDF 缩略图完整显示且不被 `cover` 裁切，缩略图内容与当前 PDF 一致，页面不重复嵌入整页 PDF 插件视图。浅色和深色下确认 Hero 姓名是唯一高强度展示元素，侧栏与主体外框保留，内部履历只使用透明底、日期脊柱和顶部细分隔，不恢复嵌套普通卡片。自动化或人工测试不得点击 PDF 链接；本机下载管理器可能接管该导航。
