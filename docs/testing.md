...
- 12 个 sitemap URL；
- `scripts/validate-site.test.js` 按稳定顺序聚合四个领域模块，共包含 320 个零依赖 `node:test` 用例，其中结构化数据专项为 79 个；
...
`run-validator-tests.js` 在四个独立 Node 进程中运行互斥分片，是 320 个验证器用例的本地全量入口。每片默认限时 5 分钟；超时先发送 `SIGTERM`，5 秒后仍未退出则发送 `SIGKILL`，强杀后 1 秒仍没有 `close`/`error` 时销毁管道、解除进程引用并以失败结算。运行器会把父进程的 `SIGINT`/`SIGTERM` 转发给存活分片，分片忽略首个信号时自动使用同一强杀链，不要求用户再次中断；最终分别以 130/143 退出，并在所有路径移除信号监听器。成功时只输出每片和全量摘要；失败时保留对应 TAP 与 stderr 供定位。
...
[GitHub Actions 测试工作流](../.github/workflows/tests.yml)在拉取请求、`main` 推送和手动触发时使用 Node.js 24。验证器的 320 个用例按注册序号稳定分为四个互斥分片，每片各包含 80 个用例；四个矩阵任务均通过同一运行器验证摘要，四片并集必须恰好覆盖全量测试。独立合同任务运行分片基础设施回归、统计客户端、Worker/D1 与站点验证入口。
...
未设置 `VALIDATOR_TEST_SHARD` 时，运行器并行运行全部四片；直接运行聚合测试文件时仍一次串行运行全部 320 个用例。新增、删除或调整用例数量时，必须同步更新 `scripts/validator-test-shard.js` 中的总数合同、本节基线和四个分片的预期数量。
...
模块加载顺序属于分片合同：调整顺序会改变用例所属分片。移动或新增用例后，必须重新核对 320 个总数、四片各 80 个的库存，以及并行入口的全量并集。

包含多次完整仓库验证的表驱动变异应拆为多个顶层测试组，使现有取模分片可以分散长尾，并通过断言消息保留具体变异名称；不要把全部昂贵场景重新合并成一个顶层测试。

共享夹具在每个测试进程中只读取一次公开运行时文件和验证器语法入口，随后为每个用例独立落盘；不得跨用例共享可变目录，也不得使用会回写源文件的硬链接。验证器只检查存在性和路径大小写的字体、普通图片与 PDF 使用空占位文件；四个解析内容的图标、六张受文件大小合同约束的简历响应式照片、HTML、CSS、JS、manifest、robots 和 sitemap 保留真实字节。测试源码、普通文档、CI 配置、`.baoyu-skills/` 与 Wrangler 本地状态不进入夹具；真实仓库基线和显式未登记 HTML 用例继续覆盖全仓库存边界。
...
分片基础设施测试的 Node 原始摘要应包含 `tests 22`、`pass 22` 和 `fail 0`；串行回退应包含 `tests 320`、`pass 320` 和 `fail 0`。并行入口只有在四片的 TAP 摘要均为 `tests 80`、`pass 80`，且四类例外计数全零后，才输出上面的紧凑成功摘要。

结构化数据名称子集的验收记录应汇总为 `79 passed / 0 failed`，图标名称子集汇总为 `47 passed / 0 failed`，历史 localStorage 子集汇总为 `8 passed / 0 failed`，档案与证明滑轨名称子集汇总为 `13 passed / 0 failed`，首页移动卡片子集汇总为 `10 passed / 0 failed`，规范本地访问历史子集、首页引文子集与英文术语子集分别汇总为 `2 passed / 0 failed`，不要把这些行写成 Node 原始输出。不同 Node 版本或执行方式仍可能把名称过滤掉的用例计入 TAP 总数；结构化数据子集此时会显示 `320 tests`、`79 pass`、`241 skipped`。
...
- 中英文首页各自只保留一份 `.quote-text` 引文，不再在 Hero 侧栏复制 `.poem-note`，避免同页重复内容与单边漂移；每页 Hero 恰有一个 `.hero-side-surface` 直接包含唯一的 `.hero-side`，其四张直属卡片依次承载身份、研究方法叙述、工具维护叙述与快捷入口。唯一的 `.home-quick-card` 必须直接包含唯一的 `.home-quick-links`，四个入口依次指向研究、项目、个人档案与 GitHub；首页不得出现 `.meta-card--stats`、`.hero-stats`、统计 endpoint 或 `stats.js`。Hero 后依次为带 `h2#home-current-title` 的浅灰当前研究、带 `h2#home-updates-title` 的白色近期状态、带 `h2#home-beyond-title` 的浅灰研究之外，以及唯一的深色 `.quote-band`；三个正文 `section` 的 `aria-labelledby` 必须精确引用各自可见标题。
...
- `stats.js` 只加载在中英文统计页；
- 两个统计页包含站点访问、本月独立设备估算、当前页面访问与本地计数 DOM；
- 两个统计页的 `#stats-status` 初始为 `loading`，并提供 `role="status"`、礼貌实时播报与原子更新；
- 两个统计页各自恰有一个批准的 Worker endpoint meta 和对应源 preconnect；
- 其他页面不误加载统计脚本；
...
验证器继续要求 `site.css` 在样式表顶层为精确选择器声明五组全局收缩保护：共享文档卡片 `min-width: 0`，简历侧栏联系方式行 `min-width: 0`，其值同时使用 `min-width: 0` 与 `overflow-wrap: anywhere`，关键词标签同时使用 `min-width: 0`、`white-space: normal` 与 `overflow-wrap: anywhere`，简历主体长小按钮使用 `max-width: 100%` 与 `white-space: normal`。这些保护仍服务共享组件和历史回归，但简历新的材料结构不得以 `.doc-grid`、`.doc-card` 或 `.proof-grid` 作为存在条件；简历材料必须是三个可收缩链接。声明按源码顺序、选择器特异性和 `!important` 优先级解析，并把选择器列表及组合符周围的等价空白规范化；注释、字符串、错误选择器、错误值，以及顶层或 `@media` / `@supports` 中能够覆盖保护值的规则，均不能满足合同。较低优先级的共享基础样式、条件规则中的无关属性、最终 subject 上仅由类型/ID/类组成的单 compound `:not(...)` 明确排除目标的选择器和只作用于伪元素的规则不应被误报。验证器不尝试实现完整浏览器选择器引擎：祖先否定、属性或通配符否定，以及带组合符、嵌套函数或状态条件的复杂函数选择器，若其受保护声明足以覆盖基线，则保守拒绝。重叠候选也逐条受约束：高优先级错误声明即使随后由安全值恢复，仍视为陈旧风险并拒绝；应删除错误声明，不依赖后续补救。其他页面的证明图如需横向滚动，仍只能发生在各自 `.proof-grid` 内，不能转移为页面级横向滚动。

### 3.10 首页移动 Hero 样式合同

验证器合并样式表顶层全部精确 `(max-width: 640px)` 媒体块，并按最终有效直接声明检查首页移动 Hero：右侧 `aside` 必须使用 `.hero-side-surface` 语义钩子并直接包含唯一的 `.hero-side`，背景使用该钩子和低密度 `48px` 网格，不接受 `:nth-child(...)` 位置回退；滑轨自身必须完整声明横向 Flex、滚动、吸附、滚动条隐藏和内边距，不依赖另一个媒体块补齐。两端 gutter 取 `28px` 与左右 safe inset 的最大值，卡宽为容器减去两侧动态 gutter；普通零 inset 视口仍等价于容器减 `56px`。滑轨继续使用 `12px` 间距、中心吸附、交叉轴拉伸与统一 `216px` 最低卡高。卡片高度保持 `auto` 且不设置上限，使普通矩形视口首卡居中时露出下一卡 `16px`，并允许长内容撑开整组卡片；身份卡保留双列布局并把联系方式降为可换行信息行；两张叙述卡的正文允许正常换行；快捷入口使用唯一 `.home-quick-card > .home-quick-links` 的 2×2 网格，入口允许换行并保持至少 `44px` 触控高度。注释、字符串、错误媒体条件、旧位置选择器、旧满宽卡片、旧宽度上限、固定卡高、尺寸分叉、恢复关键词胶囊或统计分栏，以及后续有效覆盖均不能满足合同。该静态检查不模拟真实滚动条、字体度量、触摸惯性或主题颜色，仍须按浏览器矩阵验证。

### 3.11 项目网格样式合同

验证器把项目页视为一个由层级、顺序和响应式布局共同组成的合同：正文必须依次包含 3 个代表案例、4 个本地智能与知识工具、2 个软件工程实践和 1 个开发基础设施项目；四个章节的稳定锚点、Hero 公开边界说明、完整 `data-project-id` 顺序和 JSON-LD `ItemList` 必须一致。每张卡片最多四个可见技术标签。MicFamily 项目页采用公开仓库可核验口径，不得恢复 `uni-app`、精确依赖版本或“严格前后端分离”；CBF-Rover 项目页不得把内部 `exact` / `q-SBC` 状态写入公开名称、说明或关键词。

样式方面，`.project-showcase` 保持单列案例流，每个 `.project-case` 在桌面使用约 `55% / 45%` 的正文—路径图双栏，所有直属子项具备 `min-width:0`，并在精确 `<= 833px` 媒体合同中回落单列。其余项目使用 `.project-grid--index` 的 `auto-fit` 响应式网格和 `30rem` 最小轨道，`.project-card` 保持 `min-width:0`；`auto-fill`、Flex 替代、错误比例、缺少收缩保护或断点未折叠均不能满足合同。静态检查不计算实际字体度量、卡片高度或页面横向溢出，仍须在关键断点使用浏览器核对。
...
中英文简历必须共同满足以下结构合同；未由静态验证器覆盖的部分使用源码核对、浏览器检查和打印预览完成验收：

- `.resume-hero h1` 的字号上限为 `68px`，桌面 Hero 上下留白约 `56–64px`，且不恢复视口比例最小高度；普通桌面首屏可看到在线履历开头。`.resume-page` 的屏幕基础字号为 `16px`，不允许通过全站 `body` 或缩放变换完成收缩。
- Hero 后恰有六项锚点，按顺序指向教育、科研、项目与实习、荣誉、组织与实践和材料的六个可见标题；移动端滚动只发生在锚点栏内部。
- `>= 1069px` 的主体继续使用 `280–320px` 侧栏和可收缩正文，不能因新增锚点或打印规则扩大侧栏。
- 材料区恰有三个可见入口：PDF 简历、本科成绩单 PDF、对应语言的完整个人档案与证明材料；不得把内容尚未同步的 PDF 称作“当前版本”，也不得残留 PDF 整页预览、成绩单图片、成绩单 Lightbox 或空文档卡片。
- 打印规则由 `.resume-page` 或简历命名页面限定，并以 A4 纵向检查；打印时站点导航、抽屉、锚点、按钮、材料链接、页脚和返回顶部不可见，屏幕装饰与 sticky 被移除，履历正文完整且条目尽量不跨页。共享页面不得被简历专用重排选择器命中。
- 下载版 `docs/Shibo-Yan-Resume.pdf` 必须保持为用户提供的一页 A4 原文件，不得由在线简历打印、重新排版或重新导出。当前批准文件为 `291213` 字节，SHA-256 为 `0656540EECF90265D7BB451D4E91B337D34D2988C7922D3F3BD3D01DA9B12F36`；静态验证器必须拒绝任意字节漂移。发布前用 Poppler 渲染唯一页面，检查无裁切、重叠、乱码或不可读小字。在线简历的事实与响应式验收继续独立执行，不要求下载 PDF 随网页事实更新。

事实核对不得只比较中英文页面彼此一致，还要回到正式材料与用户确认：

- 本科 GPA 为 `3.95 / 5.00`，学位课程排名为 `8 / 120`；拒绝 `8 / 124`、未说明口径的“专业排名”或从旧 PDF 反向覆盖成绩单。
- 完整个人档案中的事实仍为：2024 年研究生学业奖学金一等奖、第二十一届华为杯二等奖；2025 年研究生学业奖学金二等奖、第二十二届华为杯三等奖。在线简历与下载 PDF 的荣誉选材单独服从源 PDF：只保留 2024 华为杯、2020 认证杯、2021—2022 全国大学生数学竞赛、2019—2022 本科其他荣誉，以及含学术科技创新先进个人与一等奖学金的 2024—2025 研究生其他荣誉，并保持这一顺序；不得把档案的完整条目自动扩充到紧凑简历。
- 根据后续用户授权，MicFamily / KTV 在项目页、在线简历和个人档案中统一采用公开实现口径：Vue / Spring Boot / MyBatis-Plus / MySQL，不写 `uni-app`、精确版本或严格前后端分离。在线简历继续显示 `2022.06—2022.08` 实习期、团队开发联调与 94 分；档案和项目页继续显示 `2022.06—2022.07` 项目开发期。验证器的禁止检查必须限定在唯一 KTV 条目内，不得误伤 PersevereStudy 考勤项目及个人技能中已有依据的 `uni-app`。
- 组织与实践章节必须按 PDF 顺序保留 TIPCC、基层服务青年优才、学生会、科研助理和百度松果菁英班五项及其日期、角色；科研助理不得在科研章节重复。该区条目名称和角色为 `font-weight:400`，但保留 `h3` 语义层级；删除任一条目、交换顺序或恢复粗体均应被验证器拒绝。
- 2020 年建模奖项必须按证书写作第九届“认证杯”国际数学建模竞赛 Honorable Mention / Certificate Authority Cup，不得写作 MCM/ICM。
- CBF-Rover 的在线 CBF 安全过滤与 fixed-geometry q-SBC 分开陈述：简历与档案可按各自事实合同说明内部 `exact` / Gazebo 经验验证和仍为 `not_ready` 的 q-SBC，但不得写成形式化概率证明。项目页使用面向访问者的公开表述，只写数值动力学仿真、Gazebo、在线 CBF 与仍在推进的形式化证书模块；可见标签和 JSON-LD 均不使用内部 `exact` / `q-SBC` 状态词。
- CCF A 类会议投稿在本轮简历中原样保留完整题目 *Formal Reach-Avoid Controller Synthesis for Stochastic Systems via Iterative Neural-Symbolic Learning*、中文“在投”与英文 `Submitted` 状态标签，以及“第二作者（导师第一作者）”作者顺序。该用户授权意味着简历验收不得把它替换成匿名概述或随其他论文改写状态；研究页及其他未授权材料仍按各自公开边界检查。

### 3.14 移动基础、主题首帧与响应式照片合同

验证器把本轮移动优化视为共享基础合同，而不是某一页的补丁：

- 14 个页面都必须恰有一个包含 `width=device-width`、`initial-scale=1.0` 和 `viewport-fit=cover` 的 viewport；每页只加载一次 `assets/js/site.js`，且该脚本不带 `defer`/`async`、位于首张阻塞样式表之前。`site.js` 中恰有一次绘制前主题调用，必须在 `DOMContentLoaded` 接线之前同步设置 `data-theme` 与 `theme-color`；注释、字符串或延后调用不能满足合同。
- `site.css` 必须定义四向安全区、`--header-total-h` 和共享 `--anchor-h`，并让页头、抽屉、锚点栏、档案时间线舞台、页脚、返回顶部、Lightbox、简历区域与滚动偏移实际消费相应变量。浅色 `--ink-muted-48` 与 parchment 背景按 WCAG 相对亮度公式至少达到 `4.5:1`。`<= 833px` 的锚点栏为 `48px`、锚点触控目标至少 `44px`，页头与锚点栏关闭模糊且使用不透明表面；`<= 760px` 的简历侧栏进入紧凑纵向布局；后置 640px/419px 规则仍须保留安全区，并分别收敛移动引文。
- 当前锚点统一使用 `aria-current="location"`。`site.js` 必须选择真实滚动容器 `.anchor-bar`，在横向溢出时计算受 `0..scrollWidth-clientWidth` 约束的目标位置，并按减少动画偏好选择即时或平滑滚动；无 `scrollTo` 时保留 `scrollLeft` 回退。静态合同不证明浏览器最终几何可见性，仍须滚到最后章节实测。
- 中英文简历必须使用相同的 `<picture>` 资源集：240/480/960 像素宽 AVIF、同尺寸 WebP 和原始 `resume-photo.jpg` 回退。两种 `<source>` 与 `<img>` 都声明同一 `sizes`；派生文件必须存在、非空，并分别满足 20/40/80 KiB 的 AVIF 上限和 20/40/100 KiB 的 WebP 上限。验证器不解码 AVIF/WebP，也不替代浏览器 `currentSrc`、`naturalWidth` 和传输量检查。
...
- 全站 `viewport-fit=cover`、样式表前同步主题、四向安全区及其后置窄屏级联、浅色次要文字对比度、48px 移动锚点栏、760px 简历侧栏、当前锚点视觉状态和真实 `.anchor-bar` 跟随，以及中英文简历响应式照片标记、文件缺失、空文件和大小预算变异；
...
- 首页移动 Hero 的容器卡宽、统一卡高、身份信息行、两张叙述卡、快捷入口网格、入口换行与低密度背景网格；首页结构扫描跳过注释、模板及 raw-text/RCDATA 内容，正确处理属性引号内的 `>`，并用独立变异拒绝 Hero 外快捷卡 decoy、快捷 hook 转移或重复、统计卡回归、章节错位 decoy、错误 `aria-labelledby` 目标、缺失标题和非 `h2` 标题；
...
简历页还需在中英文 `320px`、`375px`、`640px`、`700px`、`760px`、`761px`、`768px`、`833px`、`834px`、`1068px`、`1069px` 与 `1440px` 检查：`documentElement.scrollWidth === clientWidth`；Hero 姓名不超过 `68px`，桌面上下留白约 `56–64px`，普通桌面首屏能看到在线履历开头；Hero 后六项锚点顺序和目标正确，移动端锚点栏可横向滚动且不撑宽页面。滚到最后章节时必须恰有一个 `aria-current="location"`，其左右边界完整落在 `.anchor-bar` 内，溢出场景的 `scrollLeft > 0`。`>= 1069px` 为 `280–320px` 侧栏与主体双栏，`<= 1068px` 回落单列且侧栏不保持 sticky；`761–1068px` 的侧栏为正式照片与两列元信息，`<= 760px` 改为紧凑照片/身份横排和自然纵向元信息。主体按 01–05 顺序显示教育、科研、项目与实习、荣誉、组织与实践，桌面日期列清晰，`<= 640px` 日期位于正文上方；五项组织实践完整且名称和角色为正常字重，中英文长论文标题、组织名称、角色说明、学校邮箱、关键词和长按钮均落在父容器内。简历 Hero 与侧栏使用学校邮箱和已批准入口，不重新出现旧电话或微信；全站页脚与结构化数据继续遵守既有公共身份合同。材料区只显示 PDF 简历、本科成绩单 PDF、完整个人档案与证明材料三个入口，不显示文档卡片、PDF 缩略图、成绩单图片或 Lightbox；三个入口在所有视口完整可见并保持至少 `44px` 触控高度。3:4 正式照片不变形；在 320/640/768px 及 DPR 1/2 检查 `currentSrc` 不回退到 1.6 MB 原 JPEG、`naturalWidth > 0`，并记录实际资源传输量。浅色和深色下确认 Hero 姓名是唯一高强度展示元素，侧栏与主体外框保留，内部履历只使用透明底、日期脊柱和顶部细分隔，不恢复嵌套普通卡片。自动化或人工测试不得点击 PDF 链接；本机下载管理器可能接管该导航。

简历打印另在中英文浏览器打印预览中检查 A4 纵向、正常缩放和至少两种主流浏览器：页面不得出现站点页头、抽屉、锚点、交互按钮、材料链接、页脚、返回顶部、背景网格或阴影；侧栏不 sticky，正文不裁切，论文和经历条目尽量保持完整，不产生空白首页或页面级横向溢出。随后打开一个非简历路由的打印预览，确认简历专用布局规则没有越过 `.resume-page` 作用域。打印预览不等于当前 PDF 内容正确，PDF 仍需独立逐页渲染核对。
...
项目页还需在中英文、浅色与深色主题，以及 `320px`、`375px`、`640px`、`833px`、`834px`、`1068px`、`1069px` 与 `1440px` 检查：四个锚点准确落到 3 / 4 / 2 / 1 章节，滚到末节后当前标签完整进入 `.anchor-bar` 可视区；10 个项目的可见顺序与 JSON-LD 一致，Hero 公开边界不再占用独立章节。三个代表案例在 `>= 834px` 呈正文约 55%、路径图约 45% 的双栏，在 `<= 833px` 按正文—路径图回落单列；四步路径在 320–640px 保持纵向 DOM 顺序，不强制 2×2，编号、图注、长英文、最多四个标签和全部操作入口均留在卡片内。其余索引在可容纳时形成紧凑两列，单项基础设施保持常规卡片宽度，窄屏全部回落单列；页面必须满足 `documentElement.scrollWidth === clientWidth`。考勤和 MicFamily 的证明图只作为次级 Lightbox 入口，鼠标点击、键盘 Enter 与 Escape 行为保持可用；不得把证明材料恢复为分类或主要视觉。

首页还需在中英文 `320px`、`375px`、`419px` 与 `640px` 检查：左侧研究与简历两个直接入口完整、可点击且不溢出；`.hero-side-surface` 背景不随 DOM 位置改变，四张 `.hero-side` 直属卡片等宽等高且逐张居中吸附，首卡居中时仍能看到下一卡边缘，触屏横向滑动不会变成页面级横向滚动；身份信息、两张叙述卡正文与四个快捷入口均落在卡片内，入口为 2×2 网格且触控高度不小于 `44px`。当前工作区的四步研究流程必须按 DOM 顺序完整可读，桌面呈单行链路，`<= 833px` 转为纵向且不产生页面级横向滚动，英文长节点可以自然换行。在 200% 浏览器缩放或等效长文本压力下，四卡可以一起高于 `216px`，仍保持等高且正文不被固定上限裁切。浅色和深色下检查卡片、快捷入口、研究流程、分隔线与背景网格；近期状态的每条标签应在 `<= 640px` 位于正文上方，使中英文长文本使用完整卡片内容宽度。中英文正文应依次呈现浅灰当前研究、白色近期状态、浅灰研究之外；引文作为唯一的深色收尾锚点，主文应明显亮于署名。桌面和移动宽度都要确认三个正文标题可见、内容职责清楚，首页不出现统计卡或发出统计请求。同时在 `1068px`、`1069px` 及普通桌面宽度确认 Hero 两栏共用连续画布，左右或上下都没有独立白灰底板接缝，研究标签只占内容宽度；检查完整章节节奏、引文内容驱动的上下留白、文字完整性和页面横向溢出，不要求两种语言具有相同像素高度。

全站色调验收不得只看当前 DOM 顺序：共享 CSS 不应包含为直属 `.section-block` 自动交替底色的 `:nth-of-type` 规则。中英文研究页应为“浅灰 Hero、白、浅灰、白、浅灰、白”，项目页为“浅灰 Hero、白、浅灰、白、浅灰”；简历页为“浅灰 Hero、白、浅灰”，统计页为“浅灰 Hero、白、浅灰、白”。档案时间线使用独立的透明舞台与卡片体系，不参与章节交替；正文中的 `.tile-dark` 只应出现在中英文首页引文。
...
研究页还需在中英文、浅深主题以及 `320px`、`375px`、`640px`、`833px`、`834px`、`1068px`、`1069px`、`1440px` 检查：五个页内锚点精确落到背景、核心问题、研究现状、研究意义与成果；滚到成果时当前标签完整进入 `.anchor-bar` 可视区，减少动画模式下即时跟随。背景闭环对象和状态空间图的文字、轨迹、集合与图注可辨认，状态空间图为唯一高强度视觉且不出现论文方法模块；两条公式只在各自可聚焦容器内横向滚动，`documentElement.scrollWidth === clientWidth`。`<= 1068px` 图文区域回落单列，`<= 640px` 状态空间图先于公式出现且图、公式均不撑宽页面。四个问题、三条研究脉络、三项意义、基金项目和论文清单保持自然阅读顺序；英文论文题目、基金名称、元数据和三个相关入口不得溢出。匿名 CCF A 类会议条目只能显示高层研究主题、类别与投稿状态，不出现完整题目、作者顺序、会议真名、方法名、算法、实验或评审信息。
...
1. 保存 dark + 系统 light、保存 light + 系统 dark 时，保存值在首张样式表生效前优先；无保存值时分别跟随系统 dark/light。
2. 延迟后续页面解析或在冷缓存下观察首帧，正文可绘制时根元素和 `theme-color` 已是目标主题，不先出现浅色画布；`site.js` 到达交互阶段后不得二次错误翻转。
3. 切换后根元素 `data-theme`、按钮状态和 `theme-color` 同步；刷新与跨页面导航后 `ysb-theme` 保持。
4. localStorage 读取不可用时页面不抛错，并按系统主题降级。
5. 浅色与深色下检查正文、链接、边界、卡片、抽屉和 Lightbox；浅色次要文字在 parchment 上保持可读。

PWA 安全区域的静态检查只能证明变量、消费者和零值退化。还必须在带刘海/圆角的 iPhone 竖屏与横屏、添加到主屏幕后 standalone、带显示切口的 Android，以及抽屉打开后旋转设备的场景，确认页头、抽屉、锚点栏、左右正文、页脚、Lightbox 和返回顶部不进入不可用区域。桌面浏览器的零 inset 结果不能写成非零安全区已验收。
...
- 两个统计页面各只向批准 endpoint 发出一次 JSON `POST`，请求 path 与当前页面一致，响应带 `no-store`；首页不发出统计请求；
...
无论 Worker 状态如何，统计页主体、本地计数和明确的降级文案都不应崩溃。不要把两个统计页的本地累计写成 14 页全站累计，也不要把月度设备估算写成真人独立访客。
...
- 不出现在 sitemap；
- 404 页面在跳转前不请求外网或 PDF，除主文档预期 404 外没有资源 4xx、脚本异常或新增控制台错误；跳转到首页后也不应出现统计请求。
...
| `stats.js` | 最小验证入口 | Worker 成功/失败/超时、严格响应、localStorage 与两个统计页范围 |
...
- 任一页面缺少 `viewport-fit=cover`、把共享主题脚本移到阻塞样式表之后或恢复 `defer`，深色首帧先绘制浅色画布，后置窄屏规则覆盖安全区，当前锚点无 `aria-current="location"` 高亮或滚到末节后仍留在横向栏可视区外；
- 非统计页加载 `stats.js` 或统计专用依赖；
- 两个统计页 endpoint 漂移、旧公开计数运行时或隐藏计数节点重新出现，或首页重新加载统计依赖；
...
- 首页移动 Hero 重新使用窄幅上限、固定高度上限、不同卡高、关键词胶囊/统计卡，或在 320px 让叙述正文、英文快捷入口溢出；
- 简历 Hero 重新放大到超过 `68px`、简历页正文恢复为 17px 或组织实践条目恢复粗体、Hero 恢复过高留白，六项锚点缺失或目标错位，桌面侧栏超出 `280–320px`，材料区恢复 PDF/成绩单大图预览，或简历打印规则影响非简历页面；
- 简历在 `641–760px` 恢复狭长的照片加两列元信息布局，或中英文 `<picture>` 缺少任一 AVIF/WebP 尺寸、`sizes` 漂移、移动浏览器重新选择 1.6 MB 原 JPEG；
- 简历把学位课程排名重新写成 `8 / 124`，偏离源 PDF 的五项荣誉选择或顺序、重新回填额外 2025 荣誉，在 MicFamily / KTV 中恢复 `uni-app` 或严格前后端分离、误删考勤项目及个人技能中的合法 `uni-app`，混淆在线 CBF 与尚未就绪的 q-SBC，或违背本轮“不做匿名化”的授权而删除 CCF A 投稿的完整题目、状态和作者顺序；
