...
- 12 个 sitemap URL；
- `scripts/validate-site.test.js` 按稳定顺序聚合四个领域模块，共包含 318 个零依赖 `node:test` 用例，其中结构化数据专项为 79 个；
...
`run-validator-tests.js` 在四个独立 Node 进程中运行互斥分片，是 318 个验证器用例的本地全量入口。每片默认限时 5 分钟；超时先发送 `SIGTERM`，5 秒后仍未退出则发送 `SIGKILL`，强杀后 1 秒仍没有 `close`/`error` 时销毁管道、解除进程引用并以失败结算。运行器会把父进程的 `SIGINT`/`SIGTERM` 转发给存活分片，分片忽略首个信号时自动使用同一强杀链，不要求用户再次中断；最终分别以 130/143 退出，并在所有路径移除信号监听器。成功时只输出每片和全量摘要；失败时保留对应 TAP 与 stderr 供定位。
...
[GitHub Actions 测试工作流](../.github/workflows/tests.yml)在拉取请求、`main` 推送和手动触发时使用 Node.js 24。验证器的 318 个用例按注册序号稳定分为四个互斥分片，前两片各包含 80 个用例，后两片各包含 79 个；四个矩阵任务均通过同一运行器验证摘要，四片并集必须恰好覆盖全量测试。独立合同任务运行分片基础设施回归、统计客户端、Worker/D1 与站点验证入口。
...
未设置 `VALIDATOR_TEST_SHARD` 时，运行器并行运行全部四片；直接运行聚合测试文件时仍一次串行运行全部 318 个用例。新增、删除或调整用例数量时，必须同步更新 `scripts/validator-test-shard.js` 中的总数合同、本节基线和四个分片的预期数量。
...
模块加载顺序属于分片合同：调整顺序会改变用例所属分片。移动或新增用例后，必须重新核对 318 个总数、前两片各 80 个且后两片各 79 个的库存，以及并行入口的全量并集。
...
分片基础设施测试的 Node 原始摘要应包含 `tests 22`、`pass 22` 和 `fail 0`；串行回退应包含 `tests 318`、`pass 318` 和 `fail 0`。并行入口只有在前两片的 TAP 摘要均为 `tests 80`、`pass 80`，后两片均为 `tests 79`、`pass 79`，且四类例外计数全零后，才输出上面的紧凑成功摘要。

结构化数据名称子集的验收记录应汇总为 `79 passed / 0 failed`，图标名称子集汇总为 `47 passed / 0 failed`，历史 localStorage 子集汇总为 `8 passed / 0 failed`，档案与证明滑轨名称子集汇总为 `13 passed / 0 failed`，首页移动卡片子集汇总为 `10 passed / 0 failed`，规范本地访问历史子集、首页引文子集与英文术语子集分别汇总为 `2 passed / 0 failed`，不要把这些行写成 Node 原始输出。不同 Node 版本或执行方式仍可能把名称过滤掉的用例计入 TAP 总数；结构化数据子集此时会显示 `318 tests`、`79 pass`、`239 skipped`。
...
- MicFamily / KTV 在简历与档案中保留用户确认的 `uni-app` 技术事实。在线简历采用与原 PDF 对齐的紧凑摘要，显示 `2022.06—2022.08` 实习期、团队开发联调、Spring Boot、MySQL、`uni-app` 与 94 分；项目页单独采用公开仓库可核验口径，显示 `2022.06—2022.07` 项目开发期和 Vue / Spring Boot / MyBatis-Plus / MySQL，不写 `uni-app`、精确版本或严格前后端分离。
...
简历页还需在中英文 `320px`、`375px`、`640px`、`833px`、`834px`、`1068px`、`1069px` 与 `1440px` 检查：`documentElement.scrollWidth === clientWidth`；Hero 姓名不超过 `68px`，桌面上下留白约 `56–64px`，普通桌面首屏能看到在线履历开头；Hero 后六项锚点顺序和目标正确，移动端锚点栏可横向滚动且不撑宽页面。`>= 1069px` 为 `280–320px` 侧栏与主体双栏，`<= 1068px` 回落单列且侧栏不保持 sticky；`641–1068px` 的侧栏为正式照片与两列元信息，`<= 640px` 改为紧凑照片/身份横排和自然纵向元信息。主体按 01–05 顺序显示教育、科研、项目与实习、荣誉、组织与实践，桌面日期列清晰，`<= 640px` 日期位于正文上方；五项组织实践完整且名称和角色为正常字重，中英文长论文标题、组织名称、角色说明、学校邮箱、关键词和长按钮均落在父容器内。简历 Hero 与侧栏使用学校邮箱和已批准入口，不重新出现旧电话或微信；全站页脚与结构化数据继续遵守既有公共身份合同。材料区只显示 PDF 简历、本科成绩单 PDF、完整个人档案与证明材料三个入口，不显示文档卡片、PDF 缩略图、成绩单图片或 Lightbox；三个入口在所有视口完整可见并保持至少 `44px` 触控高度。3:4 正式照片不变形。浅色和深色下确认 Hero 姓名是唯一高强度展示元素，侧栏与主体外框保留，内部履历只使用透明底、日期脊柱和顶部细分隔，不恢复嵌套普通卡片。自动化或人工测试不得点击 PDF 链接；本机下载管理器可能接管该导航。
...
项目页还需在中英文、浅色与深色主题，以及 `320px`、`375px`、`640px`、`833px`、`834px`、`1068px`、`1069px` 与 `1440px` 检查：四个锚点准确落到 3 / 4 / 2 / 1 章节，10 个项目的可见顺序与 JSON-LD 一致，Hero 公开边界不再占用独立章节。三个代表案例在 `>= 834px` 呈正文约 55%、路径图约 45% 的双栏，在 `<= 833px` 按正文—路径图回落单列；编号路径、图注、长英文、最多四个标签和全部操作入口均留在卡片内。其余索引在可容纳时形成紧凑两列，单项基础设施保持常规卡片宽度，窄屏全部回落单列；页面必须满足 `documentElement.scrollWidth === clientWidth`。考勤和 MicFamily 的证明图只作为次级 Lightbox 入口，鼠标点击、键盘 Enter 与 Escape 行为保持可用；不得把证明材料恢复为分类或主要视觉。
...
- 简历把学位课程排名重新写成 `8 / 124`，偏离源 PDF 的五项荣誉选择或顺序、重新回填额外 2025 荣誉，从 KTV 删除 `uni-app`，混淆在线 CBF 与尚未就绪的 q-SBC，或违背本轮“不做匿名化”的授权而删除 CCF A 投稿的完整题目、状态和作者顺序；
