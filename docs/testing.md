...
首页还需在中英文 `320px`、`375px`、`419px` 与 `640px` 检查：`.hero-side-surface` 背景不随 DOM 位置改变，四张 `.hero-side` 直属卡片等宽等高且逐张居中吸附，首卡居中时仍能看到下一卡边缘，触屏横向滑动不会变成页面级横向滚动；身份信息、两张叙述卡正文与四个快捷入口均落在卡片内，入口为 2×2 网格且触控高度不小于 `44px`。在 200% 浏览器缩放或等效长文本压力下，四卡可以一起高于 `216px`，仍保持等高且正文不被固定上限裁切。浅色和深色下检查卡片、快捷入口、分隔线与背景网格；近期状态的每条标签应在 `<= 640px` 位于正文上方，使中英文长文本使用完整卡片内容宽度。中英文正文应依次呈现浅灰当前研究、白色近期状态、浅灰研究之外；引文作为唯一的深色收尾锚点，主文应明显亮于署名。桌面和移动宽度都要确认三个正文标题可见、内容职责清楚，首页不出现统计卡或发出统计请求。同时在 `1068px`、`1069px` 及普通桌面宽度确认 Hero 两栏共用连续画布，左右或上下都没有独立白灰底板接缝，研究标签只占内容宽度；检查完整章节节奏、引文内容驱动的上下留白、文字完整性和页面横向溢出，不要求两种语言具有相同像素高度。
