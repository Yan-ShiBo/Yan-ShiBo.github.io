...
验证器合并样式表顶层全部精确 `(max-width: 640px)` 媒体块，并按最终有效直接声明检查首页移动 Hero：右侧 `aside` 必须使用 `.hero-side-surface` 语义钩子并直接包含唯一的 `.hero-side`，背景使用该钩子和低密度 `48px` 网格，不接受 `:nth-child(...)` 位置回退；滑轨自身必须完整声明横向 Flex、滚动、吸附、滚动条隐藏和内边距，不依赖另一个媒体块补齐，并使用容器减 `56px` 的卡宽、`28px` 两端留白、`12px` 间距、中心吸附、交叉轴拉伸与统一 `216px` 最低卡高。卡片高度保持 `auto` 且不设置上限，使首卡居中时露出下一卡 `16px`，并允许长内容撑开整组卡片；身份卡保留双列布局并把联系方式降为可换行信息行；关键词改为无间隙的 2×3 分隔矩阵；统计改为无嵌套卡框的三栏分段面板，其中 `.meta-card--stats` 使用 `18px 8px` 内边距，`.compact-stat` 使用 `10px 2px` 内边距，英文标签允许换行，计数在窄屏缩放并裁住异常超长内容。注释、字符串、错误媒体条件、旧位置选择器、旧满宽卡片、旧宽度上限、固定卡高、尺寸分叉、恢复胶囊或嵌套卡框，以及后续有效覆盖均不能满足合同。该静态检查不模拟真实滚动条、字体度量、触摸惯性或主题颜色，仍须按浏览器矩阵验证。

### 3.11 项目网格样式合同

验证器要求 `.project-grid` 使用 `auto-fit` 与响应式最小轨道宽度，普通项目最小宽度为 `22.5rem`，证明型项目组为 `30rem`，卡片本身保持 `min-width:0`。该组合让单卡占满、两卡等宽铺满，并让已经形成多列的三项以上项目组在末行继续使用相同轨道宽度；`auto-fill`、`flex-wrap`、缺少收缩保护或后续媒体覆盖均不能满足合同。静态检查不计算实际容器宽度，仍须在关键断点比较浏览器中的卡片边界。

### 3.12 首页引文层级合同

验证器要求深色引文区的 `.quote-text` 使用 `--on-dark`，`.quote-source` 使用 `--body-muted`，并拒绝重新以通用直属 `p` 颜色规则覆盖主引文。该检查锁定语义令牌与已知级联风险，不计算最终颜色或对比度；浅色与深色主题仍需在浏览器中确认主文显著亮于署名。
...
简历页还需在中英文 `640px`、`375px` 与 `320px` 检查：`documentElement.scrollWidth === clientWidth`；`.doc-grid` 不产生自身宽度溢出；`.proof-grid` 保留 `overflow-x: auto` 的内部滑轨；联系方式值、关键词标签和长小按钮均落在各自父容器内，标签文字不得覆盖相邻标签；竖版简历缩略图完整显示且不被 `cover` 裁切；页面不再重复嵌入整页 PDF 插件视图。浅色和深色下还应确认侧栏与主体外框保留，而侧栏元信息、教育和项目条目只使用透明底与顶部细分隔，不恢复嵌套的完整卡框。自动化或人工测试不得点击 PDF 链接；本机下载管理器可能接管该导航。

档案页还需在中英文桌面和 `320px` 检查：概览只显示两组邮箱且不显示电话或微信；页面本身无意外横向滚动；每个 `.proof-grid` 内的卡片宽高一致、说明文字不被裁切。浅色和深色下还应确认概览使用上下发丝线与桌面列间分隔，阶段外框保留，而阶段详情与故事只使用透明底和顶部细分隔；移动概览应改为单列并把列间分隔转换为行间分隔。另在 `640px`、`833px`、`834px`、`1068px`、`1069px` 与 `1440px` 检查高中阶段：教育卡与长荣誉卡纵向排列，宽屏荣誉列表只在自身卡片内分栏；小学单卡铺满可用宽度；本科等三项以上阶段继续保持自然多栏流。桌面用鼠标主键拖动后滑轨位置应改变且 Lightbox 不打开，随后普通点击及键盘 Enter 仍能打开对应原图；触屏保持原生横向滑动，底部滚动条继续作为回退入口。测试不得打开成绩单 PDF。

项目页还需在中英文 `640px`、`833px`、`834px`、`1068px`、`1069px` 与 `1440px` 检查：首组恰为两张带证明材料的项目卡且可见顺序与 JSON-LD 一致；两卡列宽一致，不与无媒体短卡形成跨行空洞；单卡组铺满整行，两卡组等宽铺满，三项以上普通项目的末行沿用已建立的轨道宽度，同一行卡片等高、操作入口贴近卡底；证明图保留横向滚动，鼠标点击、Enter 与 Escape 的 Lightbox 行为不变。

首页还需在中英文 `320px`、`375px`、`419px` 与 `640px` 检查：`.hero-side-surface` 背景不随 DOM 位置改变，四张 `.hero-side` 直属卡片等宽等高且逐张居中吸附，首卡居中时仍能看到下一卡边缘，触屏横向滑动不会变成页面级横向滚动；身份信息、六个关键词、三项统计标签和状态说明均落在卡片内。公开统计至少用 `--`、`0` 与 8 位连续数字复核窄屏度量；在 200% 浏览器缩放或等效长文本压力下，四卡可以一起高于 `216px`，仍保持等高且正文不被固定上限裁切。浅色和深色下检查卡片、组合面板、分隔线与背景网格。中英文概览区应使用 `.section-muted` 中性章节，卡片与章节底色需要保持可辨层级；引文作为唯一相邻的深色收尾锚点，主文应明显亮于署名，不得重新形成连续 `.tile-dark`。同时在 `1068px`、`1069px` 及普通桌面宽度确认 Hero 两栏共用连续画布，左右或上下都没有独立白灰底板接缝，研究标签只占内容宽度；在桌面和移动宽度检查整幅白色 Hero、浅灰概览、深色引文及页脚的完整节奏、引文内容驱动的上下留白、文字完整性和页面横向溢出，不要求两种语言具有相同像素高度。
...
- 首页移动 Hero 重新使用窄幅上限、固定高度上限、不同卡高、分散胶囊/嵌套统计卡，或在 320px 让关键词、英文统计标签和高位计数溢出；
