...
## 12. Favicon 更换为自定义终端验证图标

**现象**：原始 favicon (site.ico) 是默认图标，与网站的计算机科学/形式化验证主题不匹配。

**根因**：项目初始化时使用了通用图标，没有针对网站主题定制。

**解决方式**：
- 用户提供了一个包含方括号 [ ]、对勾 ✓ 和连接线的自定义图标，象征形式化验证与系统控制。
- 使用 Python PIL 将图片转换为 ICO 格式，覆盖 `assets/icons/site.ico`。
- 所有 14 个 HTML 页面通过 `<link rel="icon">` 引用同一个 `site.ico`，无需逐页修改。

**防退化检查**：
- 更换 favicon 后用 `Ctrl+F5` 强制刷新确认浏览器不使用缓存旧图标。
- 检查 `manifest.webmanifest` 中的图标路径仍指向 `assets/icons/site.ico`。
- 不要把 favicon 改成 PNG 格式引用，保持 ICO 格式兼容性。
