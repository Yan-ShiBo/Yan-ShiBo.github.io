...
中英文档案概览只公开两组已批准的邮箱链接，不公开电话号码或微信号；两页的联系入口保持一致。
...
带 `data-lightbox` 的证明图可以打开覆盖层。`.proof-grid` 继续承担自身的横向滚动；鼠标主键移动超过阈值后进入拖动状态，拖动结束产生的点击不得打开图片，未移动的点击、键盘激活和触屏原生滚动保持原有语义。打开 Lightbox 后保存触发元素、锁定背景交互并支持 Escape；关闭时恢复页面状态和原焦点。图片本身仍保留普通链接语义，脚本不可用时可以直接打开原图。
...
因此，自本版本起新增的本地累计访问只来自当前浏览器在这两个统计页上的访问；沿用同一规范键的既有浏览器，累计值仍可能包含旧版首页留下的历史访问。它不是 14 个页面的全站总访问量。其他页面既不加载统计脚本，也不应保留统计服务的专用依赖提示。
